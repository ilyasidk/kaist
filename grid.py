# ===== 9) Полный Grid Search по всем params =====
...
params = {
    'n_hidden'     : 20,        # Hidden layer size  | Range: 10 ~ 200
    'learning_rate': 0.0030,      # Learning rate      | Range: 0.001 ~ 0.1 (log scale)
    'epochs'       : 320,       # Training epochs    | Range: 100 ~ 3000
    'activation_fn': 'sigmoid', # Activation fn      | Options: 'sigmoid', 'relu', 'tanh'
    'noise'        : 0.51,       # Input noise        | Range: 0.5 ~ 1.5 (optional; for robustness test)
    'test_ratio'   : 0.2999999        # Test split ratio   | Range: 0.2 ~ 0.5 (optional; affects evaluation scale)
}
...
def train_and_eval(p):
    # 1) Данные
    X, y = make_extreme_spiral(N_POINTS, noise=p['noise'])
    X_train, X_test, y_train, y_test = train_test_split(
        X, y, test_size=p['test_ratio'], random_state=0
    )

    # 2) Активация
    if p['activation_fn'] == 'sigmoid':
        activation, activation_prime = sigmoid, sigmoid_prime
    elif p['activation_fn'] == 'relu':
        activation, activation_prime = relu, relu_prime
    elif p['activation_fn'] == 'tanh':
        activation, activation_prime = tanh, tanh_prime
    else:
        raise ValueError("Unsupported activation")

    # 3) Инициализация весов
    np.random.seed(0)
    W1 = np.random.randn(N_INPUT, p['n_hidden']) * 0.1
    b1 = np.zeros((1, p['n_hidden']))
    W2 = np.random.randn(p['n_hidden'], N_OUTPUT) * 0.1
    b2 = np.zeros((1, N_OUTPUT))
    ...
    for ep in range(p['epochs']):
        ...
        W2 -= p['learning_rate'] * dW2
        b2 -= p['learning_rate'] * db2
        W1 -= p['learning_rate'] * dW1
        b1 -= p['learning_rate'] * db1
...
for combo in itertools.product(*values):
    ...
    acc = train_and_eval(p)
    print(f"Params={p} => Acc={acc:.2f}%")
    if acc > best_acc:
        best_acc = acc
        best_params = p
